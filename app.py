...
redis_stack = RedisStack(app,'redis', vpc=vpc_stack.vpc, redissg=Fn.import_value('redis-sg-export'))
cognito_stack = CognitoStack(app,'cognito')
apigw_stack = APIStack(app,'apigw')
lambda_stack = LambdaStack(app,'lambda')
cp_backend = CodePipelineBackendStack(app,'cp-backend', artifactbucket=Fn.import_value('build-artifacts-bucket') )
notification = NotificationStack(app,'notification')
cp_frontend = CodePipelineFrontendStack(app,'cp-frontend', webhostingbucket=Fn.import_value('frontend-bucket'))
waf_stack = WafStack(app,'waf')
acm_stack = ACMStack(app, 'acm')
# cdn_stack = CDNStack(app,'cdn',s3bucket=Fn.import_value('frontend-bucket'),acmcert=acm_stack.cert_manager)
